...
## Optimized Query
- створено індекс `index_opt_orders_by_client` на (client_id, order_date) в таблиці `opt_orders` - дозволяє знаходити замовлення по клієнту і одразу фільтрувати по даті
- використано CTE, кожна таблиця фільтрується лише один раз
- підзапити змінені на JOIN
- `active_clients` винесено в окремий CTE,  рахує замовлення за весь час
### З індексом
| Metric | Value |
|--------|-------|
